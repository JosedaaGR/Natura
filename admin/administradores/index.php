...
if (isset($_GET['usuario_CodPK'])) {
	...
	if(isset($_GET['activa'])){
		...
		$admin->modificarEstado($condicion);
		print("<script>document.location.href='index.php?mensaje=• Admin activado correctamente'</script>");
	}else{
		$condicion = "usuario_Estado='No_Activo' WHERE usuario_CodPK=" . $usuario_CodPK;
		$admin->modificarEstado($condicion);
		print("<script>document.location.href='index.php?mensaje=• Admin eliminado correctamente'</script>");
	}
						...
						<div class="l-box-body">
							<div>
								<form role="form" class="form-inline pull-right" name="buscador_form" id="buscador_form" method="post" action="index.php">
									<div class="col-md-12">
										<br />
										<a href="admin_add.php" class="btn btn-primary btn-lg pull-right"><i class="fa fa-plus"></i> Añadir</a>
									</div>
								</form>
							</div>
							<div id="dataTableId_wrapper" class="dataTables_wrapper">
								<table id="dataTableId" cellspacing="0" width="100%" class="display dataTable" role="grid" aria-describedby="dataTableId_info" style="width: 100%;">
									...
									<tbody>
										<?PHP
										$sqlUsuario = "SELECT * FROM usuario WHERE usuario_Tipo='Admin' AND usuario_CodPK!=".$tuplaCom['usuario_CodPK'];
							...
							<td>" . $tuplaUsuario['usuario_Email'] . "</td>
							");
							...
							<td id=\"impresion_invisible\">
							    <a href=\"admin_mod.php?usuario_CodPK=" . $tuplaUsuario['usuario_CodPK'] . "\" class=\"btn btn-success btn-sm\"><i class=\"fa fa-pencil\"></i> Editar</a>
